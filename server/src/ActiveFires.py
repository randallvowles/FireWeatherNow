...
class ActiveFires:
    ...
    def desc_regexr(self, string):
        ...
        link_url = re.search(
            r'(http://inciweb.nwcg.gov/incident/)(.*?)(\d+)', remove_spaces)
        ...
        fire_code = (re.search(r'Fire Code:(.*?)(\w+)',
                               junk).group(0)).split(': ')
        fire_name = (re.search(r'Fire Name(.*?)(\w+\s\w+)',
                               junk).group(0)).split(': ')
        acres = (re.search(r'Acres(.*?)(\d+)', junk).group(0)).split(': ')
        start_date = (
            re.search(r'Perimeter Date(.*?)(\d+\/\d+\/\d+)', junk).group(0)).split(': ')
    ...
    def parser(self, response):
        ...
        for x in range(nkeys):
            ...
            #         'description' in this['kml']['Document']['Placemark'][x]:
            ...
            if this['kml']['Document']['Placemark'][x]['name'] not in that and \
            ...
            elif this['kml']['Document']['Placemark'][x]['name'] in that and \
                    'Polygon' in this['kml']['Document']['Placemark'][x]:

                print "Polygon"

                _coords = this['kml']['Document']['Placemark'][1][
                ...
                for y in range(npoly_els):
                    ...
                    tmp['polygon'].append(
                        {"lon": float(_junk[0]), "lat": float(_junk[1])})
                    # Append to big-o-array
                    that[this['kml']['Document']['Placemark'][x]['name'].split(" ")[
                        0]] = tmp
            else:
